...
def create_dense(inputs, units):
    ...
    dense = layers.LeakyReLU(alpha=0.01)(dense)
...
def classify(s,path_train,path_val,num_1,num_2,feature_len):
    ...
    history = CNN_model.fit(generate_data(path_train[0],path_train[1],height,width,batch_size=batch_size,num=[0]),
                            steps_per_epoch = math.ceil(25000 / batch_size), 
                            epochs=30,
                            validation_data=generate_data(path_val[0],path_val[1],height,width,batch_size=10,num=[0]),
                            validation_steps=250,
                            callbacks=[reduce_lr])
    
    CNN_model.save("./"+str(s)+".h5")
