...
def classify(path_train,path_val,num_1,num_2,feature_len,train_config):
    
    
    height = train_config["height"]
    width  = train_config["width"]
    channel_1 = train_config["channel_1"]
    channel_2 = train_config["channel_2"]
    ...
    CNN_model.compile(optimizer=keras.optimizers.Adam(lr=lr),
                      # loss=keras.losses.CategoricalCrossentropy(),  
                      # loss=keras.losses.SparseCategoricalCrossentropy(),
                      loss=['categorical_crossentropy'],
                      metrics=['categorical_accuracy'])

    history = CNN_model.fit_generator(generate_data(path_train[0],path_train[1],
                                                    height,width,
                                                    batch_size=batch_size,num=[0]),
                                      steps_per_epoch = steps_per_epoch, 
                                      epochs=epochs)
    s=0
    results=[]
    fitness=[]
    for i in generate_data(path_val[0],path_val[1],height,width,batch_size=val_batch_size,num=[0]):
        s=s+1
        if s<10:
